...
public class TableRepository {
    ...
    public void saveTable(String businessId, Table table) {


        Call<SaveTableResponse> call = ApiClient.getInstance().getApi().saveTable(businessId,table);

        call.enqueue(new Callback<SaveTableResponse>() {
            @Override
            public void onResponse(@NotNull Call<SaveTableResponse> call, @NotNull Response<SaveTableResponse> response) {

                if (response.isSuccessful()) {
                    if (response.body() != null) {
                        if (response.body().getCode() == 200) {
                            if (callBackListener != null) {
                                callBackListener.getServerResponse(response.body(), SAVE_TABLE_RESPONSE);
            ...
            @Override
            public void onFailure(@NotNull Call<SaveTableResponse> call, @NotNull Throwable t) {
                callBackListener.getServerResponse(t.getMessage(), SERVER_ERROR);
            }
        });
